  ...
  server.tool(
    ...
    async () => {
      const [
        ...
        supabase.from("jobs").select("job_number, status, updated_at, customer_id, customers(company_name)").order("updated_at", { ascending: false }).limit(10),
      ]);
      ...
      const result = {
        company: "RS PCB Assembly (R.S. Électronique Inc.), Montreal, ~$2.5M/yr, 5-6 people",
        active_customers: customerCount ?? 0,
        open_quotes: (quotesByStatus["draft"] ?? 0) + (quotesByStatus["review"] ?? 0) + (quotesByStatus["sent"] ?? 0),
        quotes_by_status: quotesByStatus,
        active_jobs: activeJobCount,
        jobs_by_status: jobsByStatus,
        outstanding_invoices: totalOutstanding,
        overdue_invoices: overdueCount,
        recent_activity: (recentJobs ?? []).map((j: any) => ({
          job_number: j.job_number,
          status: j.status,
          customer: j.customers?.company_name ?? "Unknown",
