...
export function registerJobTools(server: McpServer) {
  ...
  server.tool(
    ...
    {
      status: z.string().optional().describe("Filter by status: created, procurement, parts_ordered, parts_received, production, inspection, shipping, delivered, invoiced, archived"),
    ...
    async ({ status, customer_code, limit }) => {
      let query = supabase
        .from("jobs")
        .select("id, job_number, status, quantity, assembly_type, po_number, scheduled_start, scheduled_completion, created_at, customers(code, company_name), gmps(gmp_number, board_name)")
      ...
      const result = (data ?? []).map((j: any) => ({
        ...
        customer: j.customers?.code,
        customer_name: j.customers?.company_name,
        gmp: j.gmps?.gmp_number,
        status: j.status,
        quantity: j.quantity,
  ...
  server.tool(
    ...
    "Get full job detail with customer, GMP, quote reference, procurement status, production events, and documents.",
    {
      ...
      job_number: z.string().optional().describe("Job number, e.g. 'JB-2604-TLAN-001'"),
    },
    async ({ job_id, job_number }) => {
      let query = supabase
        .from("jobs")
        .select("*, customers(code, company_name), gmps(gmp_number, board_name), quotes(quote_number, pricing, quantities)");
      ...
      if (error || !job) {
        return { content: [{ type: "text" as const, text: `Job not found: ${error?.message ?? "unknown"}` }], isError: true };
      }
  ...
  server.tool(
    ...
    async ({ job_id, job_number, new_status, notes }) => {
      ...
      else return { content: [{ type: "text" as const, text: "Provide job_id or job_number." }], isError: true };

      const { data: job, error: jobErr } = await query.single();
      if (jobErr || !job) {
        return { content: [{ type: "text" as const, text: `Job not found: ${jobErr?.message ?? "unknown"}` }], isError: true };
      }
      ...
      if (updateErr) {
        return { content: [{ type: "text" as const, text: `Update failed: ${updateErr.message}` }], isError: true };
      }
