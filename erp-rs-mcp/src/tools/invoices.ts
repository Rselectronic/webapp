...
export function registerInvoiceTools(server: McpServer) {
  ...
  server.tool(
    ...
    {
      status: z.string().optional().describe("Filter: draft, sent, paid, overdue, cancelled"),
      customer_code: z.string().optional().describe("Filter by customer code"),
      overdue_only: z.boolean().default(false).describe("Show only overdue invoices"),
      limit: z.number().default(25).describe("Max results"),
    },
    async ({ status, customer_code, overdue_only, limit }) => {
      let query = supabase
        .from("invoices")
        .select("id, invoice_number, status, subtotal, total, tps_gst, tvq_qst, freight, issued_date, due_date, paid_date, customer_id, customers(code, company_name), jobs(job_number)")
      ...
      const result = (data ?? []).map((inv: any) => {
        ...
        return {
          ...
          customer: inv.customers?.code,
          customer_name: inv.customers?.company_name,
          job_number: inv.jobs?.job_number,
          total: inv.total,
          status: inv.status,
  ...
  server.tool(
    ...
    async () => {
      const { data: invoices, error } = await supabase
        .from("invoices")
        .select("invoice_number, total, due_date, status, customer_id, customers(code, company_name)")
      ...
      let over60 = 0;
      let over90 = 0;
      const byCustomer: Record<string, { code: string; name: string; total: number; invoices: number }> = {};
      ...
      for (const inv of invoices ?? []) {
        ...
        if (!byCustomer[custCode]) {
          byCustomer[custCode] = { code: custCode, name: cust?.company_name ?? "Unknown", total: 0, invoices: 0 };
        }
  ...
  server.tool(
    ...
    "Compare quoted vs actual cost for completed/invoiced jobs. Shows margin per job and overall summary.",
    {
      ...
      customer_code: z.string().optional().describe("Filter by customer"),
      date_from: z.string().optional().describe("ISO date, e.g. '2026-01-01'"),
    },
    async ({ job_id, customer_code, date_from }) => {
      let query = supabase
        .from("jobs")
        .select("id, job_number, quantity, status, customers(code, company_name), quotes(pricing, quantities)")
      ...
      for (const job of jobs ?? []) {
        // Get actual procurement cost
        const { data: procLines } = await supabase
          .from("procurement_lines")
          .select("extended_price")
          .eq("procurement_id", (
            await supabase.from("procurements").select("id").eq("job_id", job.id).limit(1).single()
        ...
        // Extract quoted total from pricing JSONB
        const pricing = (job as any).quotes?.pricing ?? {};
        const quotedTotal = Number(pricing.total) || 0;

        const marginPct = quotedTotal > 0 ? ((quotedTotal - actualCost) / quotedTotal) * 100 : 0;
        ...
        results.push({
          ...
          customer: (job as any).customers?.code,
