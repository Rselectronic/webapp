...
export function registerInventoryTools(server: McpServer) {
  ...
  server.tool(
    ...
    {
      m_code: z.string().optional().describe("Filter by M-Code, e.g. 'CP', '0402'"),
      low_stock_only: z.boolean().default(false).describe("Only show items with low/out-of-stock status"),
    },
    async ({ m_code, low_stock_only }) => {
      // BG stock is tracked via procurement_lines where is_bg=true
      // Aggregate across all procurements to get current stock picture
      let query = supabase
        .from("procurement_lines")
        .select("mpn, description, m_code, qty_ordered, qty_received, supplier, is_bg")
        .eq("is_bg", true);
      ...
      let items = Object.values(stockMap).map((item) => {
        ...
        // Consider low stock if received less than 20% of what was ordered
