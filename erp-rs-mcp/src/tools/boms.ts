...
export function registerBomTools(server: McpServer) {
  ...
  server.tool(
    ...
    "Get a full parsed BOM with all component lines and M-Code assignments. Look up by bom_id or by gmp_number + customer_code.",
    {
      ...
      gmp_number: z.string().optional().describe("GMP number, e.g. 'TL265-5040-000-T'"),
      customer_code: z.string().optional().describe("Customer code, required if using gmp_number"),
    },
    async ({ bom_id, gmp_number, customer_code }) => {
      ...
      if (!bomId && gmp_number && customer_code) {
        const { data: customer } = await supabase
          ...
          .single();
        if (!customer) {
          return { content: [{ type: "text" as const, text: `Customer '${customer_code}' not found.` }], isError: true };
        }
        const { data: gmp } = await supabase
          ...
          .single();
        if (!gmp) {
          return { content: [{ type: "text" as const, text: `GMP '${gmp_number}' not found for customer '${customer_code}'.` }], isError: true };
        }
        const { data: bom } = await supabase
          ...
          .single();
        if (!bom) {
          return { content: [{ type: "text" as const, text: `No BOM found for GMP '${gmp_number}'.` }], isError: true };
        }
        bomId = bom.id;
      }

      if (!bomId) {
        return { content: [{ type: "text" as const, text: "Provide either bom_id or both gmp_number and customer_code." }], isError: true };
      }
      ...
      if (error || !bom) {
        return { content: [{ type: "text" as const, text: `BOM not found: ${error?.message ?? "unknown"}` }], isError: true };
      }
  ...
  server.tool(
    ...
    "Search the master component library by MPN, description, or M-Code.",
    {
      ...
      m_code: z.string().optional().describe("Filter by M-Code, e.g. 'CP', 'IP', 'TH'"),
      limit: z.number().default(20).describe("Max results to return"),
    },
    async ({ query, m_code, limit }) => {
      let q = supabase
        .from("components")
        .select("mpn, manufacturer, description, category, package_case, mounting_type, m_code, m_code_source, digikey_pn, mouser_pn")
        .or(`mpn.ilike.%${query}%,description.ilike.%${query}%,manufacturer.ilike.%${query}%`)
        .limit(limit);
  ...
  server.tool(
    ...
    "Run rule-based M-Code classification on a single component. Checks the components DB first, then applies PAR rules.",
    {
      ...
      package_case: z.string().optional().describe("Package/case, e.g. '0402', 'SOIC-8'"),
      mounting_type: z.string().optional().describe("'Surface Mount' or 'Through Hole'"),
    },
    async ({ mpn, description, package_case, mounting_type }) => {
      ...
      for (const rule of rules ?? []) {
        const match1 = matchCondition(input, rule.field_1, rule.operator_1, rule.value_1);
        if (!match1) continue;

        if (rule.field_2 && rule.operator_2 && rule.value_2) {
          const match2 = matchCondition(input, rule.field_2, rule.operator_2, rule.value_2);
          if (!match2) continue;
        }
...
function matchCondition(
  ...
  switch (operator) {
    ...
    case "in":
      return lowerValue.split(",").map((s) => s.trim().toLowerCase()).includes(fieldValue);
    default:
      return false;
  }
}
