...
export function registerCustomerTools(server: McpServer) {
  server.tool(
    ...
    "List all customers with summary stats. Filter by active status or search by name/code.",
    {
      ...
      search: z
        .string()
        .optional()
        .describe("Search by company name or customer code"),
    },
    async ({ status, search }) => {
      let query = supabase
        .from("customers")
        .select(
          "id, code, company_name, contact_name, contact_email, payment_terms, is_active"
        );
      ...
      if (search) {
        query = query.or(
          `company_name.ilike.%${search}%,code.ilike.%${search}%`
        );
      }
      ...
      for (const j of jobCounts ?? []) {
        activeJobsByCustomer[j.customer_id] =
          (activeJobsByCustomer[j.customer_id] ?? 0) + 1;
      }
  ...
  server.tool(
    ...
    "Get full customer detail including BOM config, recent quotes, active jobs, and recent invoices.",
    {
      customer_code: z
        .string()
        .describe("Customer code, e.g. 'TLAN', 'LABO', 'CSA'"),
    },
    async ({ customer_code }) => {
      ...
      const [
        ...
        supabase
          .from("jobs")
          .select(
            "job_number, status, quantity, scheduled_completion, created_at"
          )
          .eq("customer_id", customer.id)
          .order("created_at", { ascending: false })
          .limit(5),
