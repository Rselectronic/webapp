  ...
  server.tool(
    ...
    async ({ status, customer_code, date_from, limit }) => {
      let query = supabase
        .from("quotes")
        .select(
          "id, quote_number, status, quantities, created_at, issued_at, expires_at, customer_id, customers(code, company_name), gmps(gmp_number, board_name)"
        )
      ...
      const result = (data ?? []).map((q: Record<string, unknown>) => {
        const customers = q.customers as
          | { code?: string; company_name?: string }
          | { code?: string; company_name?: string }[]
          | null;
        const c = Array.isArray(customers) ? customers[0] : customers;
        const gmps = q.gmps as
          | { gmp_number?: string }
          | { gmp_number?: string }[]
          | null;
  ...
  server.tool(
    ...
    "Get full quote detail with pricing breakdown per tier.",
    {
      ...
      quote_number: z
        .string()
        .optional()
        .describe("Quote number, e.g. 'QT-2604-001'"),
    },
    async ({ quote_id, quote_number }) => {
      let query = supabase
        .from("quotes")
        .select(
          "*, customers(code, company_name), gmps(gmp_number, board_name), boms(file_name, component_count)"
        );
      ...
      if (quote_id) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: "Provide quote_id or quote_number.",
            },
          ],
          isError: true,
        };
      ...
      if (error || !quote) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Quote not found: ${error?.message ?? "unknown"}`,
            },
          ],
          isError: true,
        };
