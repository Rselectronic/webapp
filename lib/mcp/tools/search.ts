...
export function registerSearchTools(server: McpServer) {
  server.tool(
    ...
    "Universal search across customers, quotes, jobs, invoices, and components by keyword. Returns the top matches from each entity type.",
    {
      ...
      entity_types: z
        .array(
          z.enum(["customers", "quotes", "jobs", "invoices", "components"])
        )
        .optional()
    ...
    async ({ query, entity_types, limit }) => {
      ...
      if (types.includes("customers")) {
        const { data } = await supabase
          ...
          .or(
            `company_name.ilike.%${q}%,code.ilike.%${q}%,contact_name.ilike.%${q}%`
          )
          .limit(limit);
      ...
      if (types.includes("quotes")) {
        ...
        for (const qt of data ?? []) {
          const customers = (qt as Record<string, unknown>).customers as
            | { code?: string }
            | { code?: string }[]
            | null;
          ...
          results.push({
            ...
            summary: `Status: ${qt.status} | Customer: ${c?.code ?? "N/A"}`,
          });
        }
      }
      ...
      if (types.includes("jobs")) {
        ...
        for (const j of data ?? []) {
          const customers = (j as Record<string, unknown>).customers as
            | { code?: string }
            | { code?: string }[]
            | null;
          const c = Array.isArray(customers) ? customers[0] : customers;
          results.push({
            ...
            id: j.id,
            title: j.job_number,
            summary: `Status: ${j.status} | PO: ${j.po_number ?? "N/A"} | Customer: ${c?.code ?? "N/A"}`,
          });
        }
      }
      ...
      if (types.includes("invoices")) {
        ...
        for (const inv of data ?? []) {
          ...
          results.push({
            ...
            summary: `Status: ${inv.status} | Total: $${inv.total} | Customer: ${c?.code ?? "N/A"}`,
          });
        }
      }
      ...
      if (types.includes("components")) {
        const { data } = await supabase
          ...
          .or(
            `mpn.ilike.%${q}%,description.ilike.%${q}%,manufacturer.ilike.%${q}%`
          )
          .limit(limit);
