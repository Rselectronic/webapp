...
interface DeleteNCRButtonProps {
  ncrId: string;
  ncrNumber: string;
}

export function DeleteNCRButton({ ncrId, ncrNumber }: DeleteNCRButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/ncr/${ncrId}`, { method: "DELETE" });
      if (!res.ok) {
      ...
      setError("Network error — could not delete NCR.");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This will permanently delete <strong>{ncrNumber}</strong>. This
            cannot be undone.
            {error && (
