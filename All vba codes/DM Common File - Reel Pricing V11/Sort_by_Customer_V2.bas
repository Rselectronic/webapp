Attribute VB_Name = "Sort_by_Customer_V2"
Sub CustomerSortOnly()
    ...
    ' remove any filters
    If wsDataInput.AutoFilterMode Then
        wsDataInput.AutoFilterMode = False
    End If
    
    ' Sort data in Ascending order based on Sheet Names
    sheetNamesRange.Sort Key1:=sheetNamesRange.Columns(DM_Customer_Column), Order1:=xlAscending, Header:=xlNo
